...
RectangularTrapezoid::RectangularTrapezoid(Point leftBottom, double lowBase, double topBase, double height)
...
Point RectangularTrapezoid::getCenter() const
{
  ...
  center.x_ = leftBottom_.x_ + ((lowBase_+topBase_) / 2.0);
  return center;
}
...
void RectangularTrapezoid::scale(double koef)
{
...
void RectangularTrapezoid::print() const
{
  Point center = getCenter();
  double area = getArea();
