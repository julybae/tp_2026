...
DataStruct::DataStruct() : type1(NONE), type2(NONE), double1(0), double2(0), ll1(0), ll2(0), ull1(0), ull2(0), char1(0), char2(0) {}
...
bool parseUnsignedLongLong(const std::string& s, unsigned long long& value) {
    ...
    if (str.size() > 1 && str[0] == '0' && str[1] != 'x' && str[1] != 'X' && str[1] != 'b' && str[1] != 'B') {
        try { value = std::stoull(str, nullptr, 8); return true; }
        catch(...) { return false; }
    }
    ...
    if (str.size() > 2 && str[0] == '0' && (str[1] == 'x' || str[1] == 'X')) {
        try { value = std::stoull(str, nullptr, 16); return true; }
        catch(...) { return false; }
    }
    try { value = std::stoull(str); return true; }
    catch(...) { return false; }
}
...
void parseValue(const std::string& s, DataStruct::Type& type, double& d, long long& ll, unsigned long long& ull, char& c, std::complex<double>& cmplx, std::pair<long long, unsigned long long>& rat) {
    if (parseChar(s, c)) { type = DataStruct::CHAR; }
    else if (parseDouble(s, d)) { type = DataStruct::DOUBLE; }
    else if (parseLongLong(s, ll)) { type = DataStruct::LONG_LONG; }
    else if (parseUnsignedLongLong(s, ull)) { type = DataStruct::UNSIGNED_LONG_LONG; }
    else if (parseComplex(s, cmplx)) { type = DataStruct::COMPLEX; }
    else if (parseRational(s, rat)) { type = DataStruct::RATIONAL; }
}
...
std::istream& operator>>(std::istream& in, DataStruct& data) {
    ...
    while (pos < line.size()) {
        ...
        if (keyName == "key1") {
            parseValue(keyValue, d.type1, d.double1, d.ll1, d.ull1, d.char1, d.complex1, d.rational1);
            hasKey1 = (d.type1 != DataStruct::NONE);
        } else if (keyName == "key2") {
            parseValue(keyValue, d.type2, d.double2, d.ll2, d.ull2, d.char2, d.complex2, d.rational2);
