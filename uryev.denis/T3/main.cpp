...
void handleArea(const std::vector<Polygon>& polygons, std::istream& is) {
    ...
    if (arg == "EVEN") {
        totalArea = std::accumulate(polygons.begin(), polygons.end(), 0.0, [](double sum, const Polygon& p) {
        ...
        try {
            ...
            totalArea = std::accumulate(polygons.begin(), polygons.end(), 0.0, [numVertexes](double sum, const Polygon& p) {
...
void handleCount(const std::vector<Polygon>& polygons, std::istream& is) {
    ...
    if (arg == "EVEN") {
        ...
        try {
            size_t numVertexes = std::stoull(arg);
            count = std::count_if(polygons.begin(), polygons.end(), [numVertexes](const Polygon& p) {
...
// Команда Варианта 5: ECHO <Polygon> (Дублирование вхождений)
void handleEcho(std::vector<Polygon>& polygons, std::istream& is) {
    ...
    // Наполняем новую коллекцию, дублируя совпадения
...
// Команда Варианта 5: INFRAME <Polygon> (Проверка вхождения в рамку коллекции)
...
int main(int argc, char* argv[]) {
    ...
    // Шаг 1: Изолированное чтение файла геометрии
    ...
    // Шаг 2: Обработка команд из std::cin до наступления EOF
    std::string command;
    while (std::cin >> command) {
        if (command == "AREA") {
            ...
            std::cout << "<INVALID COMMAND>\n";
            // Очистка некорректной строки ввода
            std::string dummy;
